...
// Basic access control: Administrative Assistant only (matches other admin pages)
...
if ($stmt) {
    ...
    $stmt->execute();
    $res = $stmt->get_result();
    if ($res && $res->num_rows > 0) $admin_details = $res->fetch_assoc();
    $stmt->close();
}
...
$r = $conn->query("SELECT COUNT(*) as c FROM ($status_summary_sql) docs WHERE final_status = 'Pending'");
if ($r) { $counts['pending'] = (int)$r->fetch_assoc()['c']; }
...
$recent_activities = [];
$recent_activities_sql = "SELECT da.id, da.document_id, da.assigned_by, da.assigned_to, da.status, da.assigned_at, d.title, d.tracking_number, d.office_department, u_by.first_name AS assigned_by_name, u_by.last_name AS assigned_by_lname, u_to.first_name AS assigned_to_name, u_to.last_name AS assigned_to_lname FROM document_assignments da LEFT JOIN documents d ON da.document_id = d.id LEFT JOIN users u_by ON da.assigned_by = u_by.id LEFT JOIN users u_to ON da.assigned_to = u_to.id WHERE d.document_type <> 'Travel Request'";
if ($selected_status !== '') {
    $recent_activities_sql .= " AND da.status = '$selected_status_sql'";
}
$recent_activities_sql .= " AND da.status <> 'Pending' ORDER BY da.assigned_at DESC LIMIT 15";
$r = $conn->query($recent_activities_sql);
if ($r) {
...
if (isset($_GET['export'])) {
    ...
    header('Content-Disposition: attachment; filename="' . preg_replace('/[^A-Za-z0-9_-]/','_', $escaped_dept) . '-' . preg_replace('/[^A-Za-z0-9_-]/','_', $export_type) . $filename_suffix . '.csv"');
    $out = fopen('php://output', 'w');
    if ($export_type === 'recent_uploads') {
        fputcsv($out, ['Tracking Code','Title','Status','Created At','By']);
        $q = $conn->query("SELECT * FROM ($status_summary_sql) export_docs" . ($selected_status !== '' ? " WHERE final_status = '$selected_status_sql'" : '') . " ORDER BY created_at DESC LIMIT 10");
        if ($q) {
    ...
    } elseif ($export_type === 'recent_activities') {
        fputcsv($out, ['Tracking Code','Document','Action','Assigned By','Assigned To','At']);
        $export_activities_sql = "SELECT da.id, da.document_id, da.status, da.assigned_at, d.title, d.tracking_number, u_by.first_name AS assigned_by_name, u_by.last_name AS assigned_by_lname, u_to.first_name AS assigned_to_name, u_to.last_name AS assigned_to_lname FROM document_assignments da LEFT JOIN documents d ON da.document_id = d.id LEFT JOIN users u_by ON da.assigned_by = u_by.id LEFT JOIN users u_to ON da.assigned_to = u_to.id WHERE d.document_type <> 'Travel Request'";
        if ($selected_status !== '') {
            $export_activities_sql .= " AND da.status = '$selected_status_sql'";
        }
        $export_activities_sql .= " AND da.status <> 'Pending' ORDER BY da.assigned_at DESC LIMIT 15";
        $q = $conn->query($export_activities_sql);
        if ($q) {
            while ($row = $q->fetch_assoc()) {
                fputcsv($out, [
                    $row['tracking_number'] ?? '',
                    $row['title'] ?? '',
                    $row['status'] ?? '',
                    trim((string)($row['assigned_by_name'] ?? '')) . ' ' . trim((string)($row['assigned_by_lname'] ?? '')),
                    trim((string)($row['assigned_to_name'] ?? '')) . ' ' . trim((string)($row['assigned_to_lname'] ?? '')),
                    $row['assigned_at'] ?? '',
                ]);
            }
        }
    } else {
        fputcsv($out, ['Tracking Code','Subject/Title','Sender','Description','Date Received','Classification','Sub-Classification','Prioritization','Status']);
        $q = $conn->query("SELECT * FROM ($status_summary_sql) export_docs" . ($selected_status !== '' ? " WHERE final_status = '$selected_status_sql'" : '') . " ORDER BY date_received DESC, created_at DESC");
        if ($q) {
            while ($row = $q->fetch_assoc()) {
                $senderValue = trim((string)($row['sender_name'] ?? ''));
                if ($senderValue === '') {
                    $senderValue = 'User #' . ($row['created_by'] ?? '');
                }
...
<head>
    ...
    <style>
        ...
        .reports-hero {
            position: relative;
            overflow: hidden;
            background: linear-gradient(135deg, #0d6efd 0%, #0b5ed7 55%, #66b2ff 100%);
            color: #fff;
            border-radius: 28px;
            padding: 32px;
    ...
    <div class="admin-container">
        ...
        <div class="admin-sidebar">
            <div class="admin-sidebar-header">
                <div class="admin-logo-icon"><i class="fas fa-cogs"></i></div>
                <h1>Administrative Panel</h1>
            </div>
            <nav class="admin-nav-menu">
                <ul>
                    <li><a href="admin-dashboard-staff.php" class="admin-nav-item"> <i class="fas fa-tachometer-alt"></i> <span>Dashboard</span></a></li>
                    ...
                    <li><a href="received.php" class="admin-nav-item"> <i class="fas fa-check-circle"></i> <span>Approved</span></a></li>
                    <li><a href="returned.php" class="admin-nav-item"> <i class="fas fa-undo"></i> <span>Returned</span></a></li>
                                    ...
                                    <?php if (!empty($recent_uploads)): ?>
                                        <?php foreach ($recent_uploads as $doc): ?>
                                            ...
                                            <tr data-status="<?php echo htmlspecialchars($docStatus); ?>">
                                                ...
                                                <td>
                                                    <?php echo htmlspecialchars($doc['created_by_label'] ?? ('User #' . ($doc['created_by'] ?? ''))); ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
