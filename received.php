<?php
/**
 * Approved Page - Requires Authentication
...
<body>
                ...
                <div class="page-header">
                    <div class="header-with-button">
                        <div>
                            ...
                            <p>Documents that have been approved by administrative staff</p>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody id="documentsTableBody">
                            <?php if (count($received_documents) > 0): ?>
                                ...
                                <tr id="emptyFilterRow" style="display: none;">
                                    <td colspan="10" style="text-align: center; padding: 40px; color: #999;">No approved documents</td>
                                </tr>
                            <?php else: ?>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="travelEventDate" name="event_date" required min="">
                                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set min date to today for travel event date (no past dates allowed)
            const travelEventDate = document.getElementById('travelEventDate');
            if (travelEventDate) {
                const today = new Date().toISOString().split('T')[0];
                travelEventDate.setAttribute('min', today);
            }
        });

        let currentReceivedDocument = null;
        // Global assignment state for received documents (used by preview/submit helpers)
            ...
            fetch('get-document-details.php?assignment_id=' + assignmentId)
                ...
                .then(data => {
                    if (data.success && data.document) {
                        ...
                        // Hydrate a safe global assignment object so preview/submit handlers
                        // can reliably read assignment identifiers regardless of page scope.
                        currentReceivedAssignment = {
                            assignment_id: doc.assignment_id || doc.assignmentId || doc.assignment || null,
                            id: doc.assignment_id || doc.id || null
                        };
        ...
        function loadUploadedFiles(assignmentId) {
            ...
            fetch('administrative/get-document-uploads.php?assignment_id=' + assignmentId)
                .then(response => response.json())
                .then(data => {
        ...
        function validateTravelRequestForm() {
            ...
            // Validate travel event date is not in the past
            const eventDateValue = document.getElementById('travelEventDate').value;
            if (eventDateValue) {
                const selected = new Date(eventDateValue);
                const todayDate = new Date();
                todayDate.setHours(0,0,0,0);
                selected.setHours(0,0,0,0);
                if (selected < todayDate) {
                    alert('Date of event/activity cannot be before today.');
                    return false;
                }
            }
        ...
        function previewTravelRequest() {
            ...
            window.pendingTravelRequest = {
                ...
                document_type: 'Travel Request',
                assignment_id: cleanHTML((currentReceivedAssignment && (currentReceivedAssignment.assignment_id || currentReceivedAssignment.id)) ? (currentReceivedAssignment.assignment_id || currentReceivedAssignment.id) : ''),
        ...
        function buildTravelRequestPayload() {
            ...
            const payload = {
                ...
                priority: cleanHTML(document.getElementById('relatedPriority').value || ''),
                document_type: 'Travel Request',
                assignment_id: cleanHTML(assignmentId),
                parent_document_id: cleanHTML(document.getElementById('parentDocumentId').value || ''),
        ...
        function submitTravelRequest() {
            ...
            const storedPayload = previewModal?.dataset?.pendingTravelRequest;
            const pendingPayload = window.pendingTravelRequest || (storedPayload ? JSON.parse(storedPayload) : null) || buildTravelRequestPayload();

            if (!pendingPayload || !pendingPayload.title) {
                alert('Form data lost. Please preview again.');
                return;
            }

            window.pendingTravelRequest = pendingPayload;
            if (previewModal) {
                previewModal.dataset.pendingTravelRequest = JSON.stringify(pendingPayload);
            }
