<?php
/**
 * Mayor Documents Page
 * Read-only view of documents for Mayor role
 */
...
// Primary source is registered users; assignments are included as fallback.
$offices = [];
$office_result = $conn->query("SELECT office_department FROM (
    SELECT DISTINCT office_department FROM users WHERE office_department IS NOT NULL AND office_department != ''
    UNION
    SELECT DISTINCT office_department FROM document_assignments WHERE office_department IS NOT NULL AND office_department != ''
) office_list
WHERE LOWER(TRIM(office_department)) != 'administrative office'
ORDER BY office_department");
if ($office_result) {
...
if ($office_filter) {
    $document_query .= " AND (
        da.office_department = ?
        OR u_assigned.office_department = ?
        OR u_creator.office_department = ?
        OR d.office_department = ?
    )";
...
// Keyword search across document and assignment fields
if (!empty($q)) {
    $document_query .= " AND (
        ...
        OR d.classification LIKE ?
        OR d.sub_classification LIKE ?
        OR d.priority LIKE ?
        OR da.status LIKE ?
        OR d.status LIKE ?
        OR da.office_department LIKE ?
        OR CONCAT(COALESCE(u_creator.first_name, ''), ' ', COALESCE(u_creator.last_name, '')) LIKE ?
        OR CONCAT(COALESCE(u_assigned.first_name, ''), ' ', COALESCE(u_assigned.last_name, '')) LIKE ?
    )";
    $likeQ = '%' . str_replace(' ', '%', $q) . '%';
    $params[] = $likeQ;
    $params[] = $likeQ;
...
$document_query .= " ORDER BY COALESCE(da.completed_at, da.received_at, da.assigned_at, d.date_sent) DESC LIMIT 200";
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .main-content { flex: 1; margin-left: 280px; background-color: var(--bg-light); overflow-y: auto; min-height: 100vh; display: flex; flex-direction: column; }
        .page { padding: 40px; display: block; width: 100%; flex: 1; }
        .page-header { margin-bottom: 32px; display: block; width: 100%; }
        .page-header h2 { font-size: 28px; margin-bottom: 8px; color: var(--text-dark); font-weight: 700; }
        .page-header p { font-size: 14px; color: var(--text-light); }

        .data-table { width: 100%; border-collapse: collapse; background-color: var(--bg-white); border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow-md); }
        .data-table thead th { background-color: #f5f5f5; padding: 16px 12px; text-align: left; font-size: 13px; font-weight: 600; color: var(--text-dark); border-bottom: 2px solid var(--border-color); text-transform: uppercase; letter-spacing: 0.5px; }
        .data-table tbody td { padding: 12px; border-bottom: 1px solid var(--border-color); font-size: 13px; }
        ...
        .btn { padding: 10px 20px; border-radius: 8px; border: none; font-size: 14px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; text-decoration: none; }
        .btn-sm { padding: 6px 10px; border-radius: 6px; font-size: 13px; gap: 6px; }
        ...
        .filter-group select, .filter-group input { padding: 8px 12px; border: 1px solid var(--border-color); border-radius: 6px; font-size: 13px; }

        .filters-section { background-color: #fafafa; padding: 18px; border-radius: var(--radius-lg); margin-bottom: 20px; border: 1px solid var(--border-color); display: block; width: 100%; }
        .filters-grid { display: grid; grid-template-columns: repeat(6, minmax(150px, 1fr)) auto auto; gap: 12px; align-items: end; }
        .filter-label { display: block; font-size: 11px; font-weight: 700; color: #666; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.6px; }
        .input-search { padding: 10px 12px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 13px; width: 100%; }
        .btn-search { background: linear-gradient(135deg, var(--primary-color), #ff8a00); color: #fff; border: none; padding: 10px 16px; border-radius: 8px; font-weight: 700; cursor: pointer; display: inline-flex; gap: 8px; align-items: center; }
        .btn-clear { background: #e9ecef; color: #333; border: none; padding: 10px 14px; border-radius: 8px; cursor: pointer; font-weight: 700; }
        .filter-label-spacer { visibility: hidden; }

        .empty-state { text-align: center; padding: 60px 20px; color: var(--text-light); }
        .empty-state i { font-size: 48px; margin-bottom: 16px; opacity: 0.5; }

        .modal { display: none; position: fixed; z-index: 2000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); overflow: auto; align-items: center; justify-content: center; }
        .modal.active { display: flex; }
        .modal-content { background-color: #fefefe; margin: auto; padding: 0; border-radius: 12px; width: 90%; max-width: 700px; max-height: 90vh; overflow-y: auto; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15); }
        .modal-large { max-width: 900px; }
        .modal-header { background-color: #f5f5f5; padding: 20px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; border-radius: 12px 12px 0 0; }
        .modal-header h3 { margin: 0; font-size: 18px; font-weight: 600; color: var(--text-dark); }
        .modal-close { background: none; border: none; font-size: 24px; cursor: pointer; color: #666; }
        .modal-body { padding: 20px; }
        .modal-footer { background-color: #f5f5f5; padding: 16px 20px; border-top: 1px solid var(--border-color); display: flex; justify-content: flex-end; gap: 12px; border-radius: 0 0 12px 12px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <main class="main-content">
            <div class="page">
                <div class="page-header">
                    <h2><i class="fas fa-folder-open" style="color: var(--primary-color); margin-right: 12px;"></i>Documents</h2>
                    <p>View documents and their latest assignment status</p>
                </div>
                ...
                <div class="filters-section">
                    <form method="GET" action="" id="advancedFiltersForm">
                        <div class="filters-grid">
                            <div>
                                <label class="filter-label">Office/Department</label>
                                <select name="office" class="input-search">
                                    <option value="">All Offices</option>
                                    <?php foreach ($offices as $office): ?>
                                        <option value="<?php echo htmlspecialchars($office); ?>" <?php echo $office_filter === $office ? 'selected' : ''; ?>><?php echo htmlspecialchars($office); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="q" class="input-search" value="<?php echo htmlspecialchars($q); ?>" placeholder="Tracking, title, sender, type, status...">
                            </div>

                            <div>
                                <label class="filter-label">Document Type</label>
                                <select name="type" class="input-search">
                            ...
                            <div>
                                ...
                                <input type="date" name="date_from" class="input-search" value="<?php echo htmlspecialchars($date_from); ?>">
                            </div>

                            <div>
                                <label class="filter-label">Date To</label>
                                <input type="date" name="date_to" class="input-search" value="<?php echo htmlspecialchars($date_to); ?>">
                            </div>

                            <div>
                                <label class="filter-label">Status</label>
                                <select name="status" class="input-search">
                            ...
                            <div>
                                <label class="filter-label filter-label-spacer">Action</label>
                                <button type="submit" class="btn-search"><i class="fas fa-filter"></i> Filter</button>
                            </div>

                            <div>
                                <label class="filter-label filter-label-spacer">Action</label>
                                <a href="documents.php" class="btn-clear" style="display:inline-flex; align-items:center; gap:8px; text-decoration:none;"><i class="fas fa-rotate-right"></i> Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            <?php foreach ($documents as $doc): ?>
                                <?php
                                    $status = (!empty($doc['assignment_status']) ? $doc['assignment_status'] : (!empty($doc['document_status']) ? $doc['document_status'] : 'Pending'));
                                    $badge_class = 'badge-pending';
                                    if (in_array($status, ['Completed', 'Approved'])) $badge_class = 'badge-completed';
                                ...
                                ?>
                                <tr data-filter-row="true" data-keywords="<?php echo htmlspecialchars(strtolower(($doc['tracking_number'] ?? '') . ' ' . ($doc['title'] ?? '') . ' ' . ($senderName ?? '') . ' ' . ($doc['description'] ?? ''))); ?>" data-sender="<?php echo htmlspecialchars(strtolower($senderName)); ?>" data-priority="<?php echo htmlspecialchars($doc['priority'] ?? 'Normal'); ?>" data-date="<?php echo htmlspecialchars(!empty($doc['date_received']) ? $doc['date_received'] : ($doc['date_sent'] ?? '')); ?>">
                                    ...
                                    <td><?php echo htmlspecialchars($doc['classification'] ?? '-'); ?></td>
                                    <td><?php echo htmlspecialchars($doc['priority'] ?? 'Normal'); ?></td>
                                    <td><span class="badge <?php echo $badge_class; ?>"><?php echo htmlspecialchars($status); ?></span></td>
                                    <td><button class="btn btn-primary btn-sm" onclick="viewRecordDocument(<?php echo intval($doc['assignment_id'] ?? 0); ?>, <?php echo intval($doc['document_id'] ?? 0); ?>)"><i class="fas fa-eye"></i> View</button></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
    ...
    <!-- (Placeholder - implement server integration if needed) -->

    <!-- Travel Request Preview Modal -->
    <!-- (Travel requests are shown inside the main view modal under Travel Requests Section) -->
